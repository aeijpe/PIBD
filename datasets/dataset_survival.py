...
class SurvivalDatasetFactory:
    ...
    def _setup_survpath(self):
        ...
        self.signatures_names = self.signatures.columns
        self.omic_names = []
        for col in self.signatures.columns:
            omic = self.signatures[col].dropna().unique()
            omic = sorted(_series_intersection(omic, self.all_modalities["rna"].columns))
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        r"""
        Given the omics_format, return the correctly transformed version of the data
        ...
        if self.omics_format == "gene": # TODO: A sequence of genes 1*N
        ...
        elif self.omics_format == "groups": # "coattn" and "motcat" use this format
        ...
        elif self.omics_format == "pathways": # "PIBD" and "survpath" use this format
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            ...
            raise NotImplementedError('Omics Type [%s] not implemented.' % self.omics_format)
        ...
        if self.sample:
            ...
            if self.mode == "cluster":
                ...
                if n_samples == max_patches:
                    ...
                    mask = torch.concat([mask, torch.zeros([how_many_to_add])])
