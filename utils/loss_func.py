...
class SurvRankingLoss(nn.Module):
    ...
    def forward(self, y_hat, T, E):
        ...
        for (idx_a, idx_b) in combinations(range(batch_size), 2):
            time_a, event_a = T[idx_a], events[idx_a]
            time_b, event_b = T[idx_b], events[idx_b]

            if time_a < time_b and event_a:
                # a and b are comparable, a is more risky
                Z_more_risky.append(y_hat[idx_a])
                Z_less_risky.append(y_hat[idx_b])

            elif time_b < time_a and event_b:
                # a and b are comparable, b is more risky
                Z_more_risky.append(y_hat[idx_b])
                Z_less_risky.append(y_hat[idx_a])
